---
title: "Элемент <handToolTypes>"
description: "Кастомные типы ручных инструментов (набор спек); пара к handToolSpecializations."
sidebar:
  label: "handToolTypes"
---
```xml
...
>
> Типы ручных инструментов задаются в modDesc через `<handToolTypes>`; движок собирает их общим
> механизмом типов (`TypeManager`, `g_handToolTypeManager`).
...
входит в открытые исходники. Известны идентификаторы `chainsaw` и `highPressureWasherLance`; точные
строки остальных базовых типов первоисточником здесь не подтверждаются (перечень спек — подтверждён,
см. раздел 3).
...
префикс мода и находит `<modName>.myTool`. Так же ссылаются на базовый тип (`type="chainsaw"`). Тип
задаётся **атрибутом корня** `<handTool type="...">`.
...
- **Тип в дочернем элементе** — тип задаётся атрибутом корня `<handTool type="...">`,
...
- Поведение — по движку FS25. Список базовых **типов** задан в
