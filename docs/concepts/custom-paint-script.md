...
> Раздел справочника — Concepts (сквозной гайд). Механика: параметр шейдера `colorScale` (FS25) /
> `colorMat0` (FS22), см. [материалы и покраска](../base/materials-paint.md).
...
- **Цвет = параметр шейдера материала** `colorScale` (FS25): `x,y,z` = линейный RGB, `w` = тип
  краски/глянца. Установка: `setShaderParameter(node, "colorScale", r, g, b, w, false)` (`w=nil` —
  сохранить текущий тип). В **FS22** тот же принцип на канале `colorMat0`.
...
-- Работает на FS25 и FS22 (покраска идёт через параметр шейдера "colorScale";
-- на FS22 замените "colorScale" на "colorMat0").
--
-- Механика:
...
- Красится параметр `colorScale` (FS25). На **FS22** заменить на `colorMat0` (там 7-арг `setShaderParameter`,
  без завершающего индекса материала).
- Имена функций сверены с исходниками FS25/FS22 (`VehicleMaterial`, `ConfigurationUtil`, событие по
