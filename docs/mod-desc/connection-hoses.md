...
- Механика подтверждена по исходникам FS22 и FS25 (`ConnectionHoseManager.lua`,
  `specializations/ConnectionHoses.lua`, `mods.lua`) и реальными модами/базовым файлом-зеркалом
  (FS25_BednarBatWing, FS25_SlurryPipeSystem, `connectionHoses.xml`). Базовый файл —
